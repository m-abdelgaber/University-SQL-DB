...
namespace GUCera.Instructor
{
    public partial class viewAssignments : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            while (rdr.Read())
            {
                string sid =  rdr.GetValue(rdr.GetOrdinal("sid")).ToString();
                string cid =  rdr.GetValue(rdr.GetOrdinal("cid")).ToString();
                string assignmentNumber =  rdr.GetValue(rdr.GetOrdinal("assignmentNumber")).ToString();
                string grade =  rdr.GetValue(rdr.GetOrdinal("grade")).ToString();
                string assignmenttype = rdr.GetValue(rdr.GetOrdinal("assignmenttype")).ToString();
