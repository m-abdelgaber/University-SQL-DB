...
go
create proc existDuplicateEmail
@email varchar(10),
@output bit output
as
...
go
create proc viewStudentCourses
@sid int
as
select stk.cid, c.name
from StudentTakeCourse stk inner join Course c on stk.cid = c.id
where stk.sid = @sid

go
create proc AssignmentExist
@cid int,
@number int,
@type varchar(10),
@output bit output
as
if(exists(select* from Assignment where cid = @cid and number = @number 
...
go
create proc AssignmentAlreadySubmitted
@cid int,
@number int,
@type varchar(10),
@sid int,
@output bit output
as
if(exists(Select* from StudentTakeAssignment where cid = @cid and sid= @sid
	and assignmentNumber =@number and assignmenttype =@type))
set @output ='1'
else
set @output ='0'
go

create proc FeedbackExist
@cid int,
@output bit output
as
...
go
Create proc checkStudentHasPromoCode
@sid int ,
@code varchar(10),
@success bit output
as
if exists(select * from StudentHasPromocode where code=@code and sid=@sid)
set @success ='1' 
else
set @success='0'
...
AS
SELECT Course.name,Course.id,Course.instructorId FROM Course LEFT OUTER JOIN StudentTakeCourse ON Course.id = StudentTakeCourse.cid 
LEFT OUTER JOIN Student ON StudentTakeCourse.sid = Student.id
WHERE Course.accepted = '1' AND StudentTakeCourse.sid IS NULL

go
create proc ViewUnacceptedCoursesByID
As
Select id , name
From Course
Where accepted = 0 or accepted is null
go
Create proc checkAdminId
@id int,
@success bit output
as
if exists(select id from Admin where id=@id)
...
create proc CheckPromoCodeExist
@code varchar(10),
@adminId int,
@success bit output
as 
if exists(select * from PromoCode where code=@code and adminId=@adminId)
set @success ='1' 
else
set @success='0' 
go

Create proc checkPromoCode
@code varchar(10),
@success bit output
as
if exists(select * from Promocode where code=@code)
set @success ='1' 
else
set @success='0'
go

create proc showStudentIds
as
select id
from Student
go
Create Proc viewContent
@id int
As
